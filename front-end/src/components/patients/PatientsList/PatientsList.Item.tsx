    ...
    <li {...liProps}>
      <Button
        className={classNames(
          ...
          'h-20 !justify-start gap-x-3 border !border-slate-300 bg-white !py-0 !px-4 shadow-sm hover:border-slate-400',
        )}
        variant="outlined"
        {...muiButtonProps}
      >
        ...
        <div className="flex flex-col items-start">
          <p className="text-left text-sm font-medium text-slate-900 sm:text-base">{fullName}</p>
          <p className="text-sm text-slate-500 sm:text-base">{email}</p>
        </div>
      </Button>
    </li>
  );
